...
import { CALC_STATS } from '../stats/reducer';
    ...
    return (action$) => {
        return action$
            ...
            .map((values) => {
                ...
                const min = Math.min(...values);
                const max = Math.max(...values);
                return Action(CALC_STATS,{ period, type, min, max, avg });
            })
    };
};
    ...
    return (action$,store) => {
        return action$
            ...
            .map((action) => {
                ...
                return Action(CALC_STATS,{ period: 'total', type, min, max, avg });
            });
    };
};
